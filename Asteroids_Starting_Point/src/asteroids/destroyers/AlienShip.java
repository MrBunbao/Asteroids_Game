...
public class AlienShip extends Participant implements ShipDestroyer, FriendlyBulletDestroyer, ActionListener {
	...
	/**
	 * The Class that manages the alien ship participants. If isSmall boolean ==
	 * true, that means the alien ship is small. The small alien is worth 1000
	 * points. 200 for the medium.
	 */
	...
	/**
	 * Then Alien ships fire timer. If small ship, it will shoot at the player.
	 * This is also the path changer of the ship.
	 */
	@Override
	public void actionPerformed(ActionEvent arg0) {
		if (!this.isExpired()) {
			if (arg0.getSource() == pathChanger) {
				...
				EnemyBullet bullet = new EnemyBullet(this.getX(), this.getY(),
						(worth == 1000 ? shipdir : (Math.PI * 2) / (1 + Constants.RANDOM.nextInt(10))));
