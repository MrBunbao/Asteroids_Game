...
public class Asteroid extends Participant
		implements ShipDestroyer, AlienShipDestroyer, EnemyBulletDestroyer, FriendlyBulletDestroyer {
	...
	/**
	 * Creates the outline of the asteroid based on its variety (variety is
	 * random) and size.
	 */
	...
	public void collidedWith(Participant p) {
		...
		if (p instanceof AsteroidDestroyer) {
			...
			if (size == 1) {
				controller.addParticipant(new Asteroid(RANDOM.nextInt(4), size, getX(), getY(),
						Constants.MEDIUM_ASTEROID_SPEED, controller));
				controller.addParticipant(new Asteroid(RANDOM.nextInt(4), size, getX(), getY(),
						Constants.MEDIUM_ASTEROID_SPEED, controller));
			} else if (size == 0) {
				controller.addParticipant(new Asteroid(RANDOM.nextInt(4), size, getX(), getY(),
						Constants.FAST_ASTEROID_SPEED, controller));
				controller.addParticipant(new Asteroid(RANDOM.nextInt(4), size, getX(), getY(),
						Constants.FAST_ASTEROID_SPEED, controller));
			}

			// Inform the controller that a bullet is hitting the asteroid. The
			// +1 will help it so it uses the right case.
